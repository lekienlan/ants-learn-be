...
export interface IPigPayload extends Omit<IPig, 'user' | 'code' | 'periodId' | 'period'> {
}
export interface IPigUpdatePayload extends Omit<IPig, 'user' | 'code' | 'periodId' | 'period'> {
    id: string;
}
export interface IPigDoc extends IPig, Document {
}
export interface IPigModel extends Model<IPigDoc> {
    paginate(filter: Record<string, any>, options: IPaginateOptions): Promise<IPaginateResult<IPig>>;
}
